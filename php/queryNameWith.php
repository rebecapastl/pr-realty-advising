...
<html lang="en">
	...
	<body>
		<header>
	        <h2>Welcome to PR Realty Advising :: Customer's name with</h2>
	    ...
	    <main>
			<?php
			    ...
			    try {
			        $sql = "SELECT CustomerID, FirstName, LastName, PhoneNumber, Email, Occupation FROM Customer WHERE FirstName LIKE '" . $_POST['name'] . "%'";
			        ...
			        if ($row) {
			            echo '<table>';
			            echo '<tr> <th>CustomerID</th> <th>First Name</th> <th>Last Name</th> <th>Phone Number</th> <th>Email</th> <th>Occupation</th> </tr>';
			            do {
			                echo '<tr><td>' . $row['CustomerID'] . '</td><td>' . $row['FirstName'] . '</td><td>' . $row['LastName'] . '</td><td>' . $row['PhoneNumber'] . '</td><td>' . $row['Email'] . '</td><td>' . $row['Occupation'] . '</td></tr>';
			    ...
			    echo "<a href='../select.html'>Back to SELECT queries</a>";
