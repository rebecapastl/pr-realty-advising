...
<html lang="en">
	...
	<body>
		<header>
	        <h2>Welcome to PR Realty Advising :: Display contact information of all customers</h2>
	    ...
	    <main>
		    <?php
			    ...
			    try {
			        $sql = "SELECT  CustomerID, FirstName, LastName,PhoneNumber, Email FROM Customer";
			        ...
			        if ($row) {
			            echo '<table>';
			            echo '<tr> <th>CustomerID</th> <th>First Name</th> <th>Last Name</th> <th>Phone Number</th> <th>Email</th> </tr>';
			            do {
			                echo '<tr><td>' . $row['CustomerID'] . '</td><td>' . $row['FirstName'] . '</td><td>' . $row['LastName'] . '</td><td>' . $row['PhoneNumber'] . '</td><td>' . $row['Email'] . '</td></tr>';
