...
<html lang="en">
	...
	<body>
		<header>
	        <h2>Welcome to PR Realty Advising :: Delete a customer</h2>
	    ...
	    <main>
			<?php
			    ...
			    try {
			        $sql = "DELETE FROM " . $dbname . ".Customer WHERE CustomerID = :customerID";
			        $stmnt = $conn->prepare($sql);
			        $stmnt->bindParam(':customerID', $_POST['customerID']);
			        $stmnt->execute();
			        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			        echo "<p style='color:green'>Customer deleted</p>";
			    } catch (PDOException $err) {
			        echo "<p style='color:red'>Customer record deletion failed: " . $err->getMessage() . "</p>\r\n";
			    }
				
				try {
			        $sql = "DELETE PostalCodes.*, Customer_Has_Address.* FROM " . $dbname . ".PostalCodes, " . $dbname . ".Customer_Has_Address WHERE PostalCodes.PostalCode = Customer_Has_Address.PostalCode AND Customer_Has_Address.CustomerID = '" . $_POST['customerID'] . "'";
			        $stmnt = $conn->prepare($sql);
			        $stmnt->bindParam(':customerID', $_POST['customerID']);
			        $stmnt->execute();
			        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			    } catch (PDOException $err) {
			        echo "<p style='color:red'>Customer's address record deletion failed: " . $err->getMessage() . "</p>\r\n";
			    }
