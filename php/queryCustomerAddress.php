	...
	<body>
	    ...
	    <main>
		    <?php
			    ...
			    try {
			        $sql = "SELECT  Customer.CustomerID, FirstName, LastName, PhoneNumber, Email, Occupation, Unit, StreetNumber, StreetName, Municipality, Province, PostalCodes.PostalCode FROM (Customer LEFT JOIN Customer_Has_Address ON Customer.CustomerID = Customer_Has_Address.CustomerID) LEFT JOIN PostalCodes ON Customer_Has_Address.PostalCode = PostalCodes.PostalCode";
			        ...
			        if ($row) {
			            echo '<table>';
			            echo '<tr> <th>CustomerID</th> <th>First Name</th> <th>Last Name</th> <th>Phone Number</th> <th>Email</th> <th>Occupation</th> <th>Unit</th> <th>Street Number</th> <th>Street Name</th> <th>Municipality</th> <th>Province</th> <th>Postal Code</th> </tr>';
			            do {
			                echo '<tr><td>' . $row['CustomerID'] . '</td><td>' . $row['FirstName'] . '</td><td>' . $row['LastName'] . '</td><td>' . $row['PhoneNumber'] . '</td><td>' . $row['Email'] . '</td><td>' . $row['Occupation'] . '</td><td>' . $row['Unit'] . '</td><td>' . $row['StreetNumber'] . '</td><td>' . $row['StreetName'] .'</td><td>' . $row['Municipality'] . '</td><td>' . $row['Province'] . '</td><td>' . $row['PostalCode'] .'</td></tr>';
